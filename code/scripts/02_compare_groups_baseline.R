...
# ANOVA
age_model <- aov(age ~ attend_cat, data = filter(baseline_df, exclude == "No"))
height_model <- aov(height ~ attend_cat, data = filter(baseline_df, exclude == "No"))
body_mass_model <- aov(body_mass ~ attend_cat, data = filter(baseline_df, exclude == "No"))
BMI_model <- aov(BMI ~ attend_cat, data = filter(baseline_df, exclude == "No"))
waist_circunference_model <- aov(waist_circunference ~ attend_cat, data = filter(baseline_df, exclude == "No"))
hip_circunference_model <- aov(hip_circunference ~ attend_cat, data = filter(baseline_df, exclude == "No"))
waist_hip_ratio_model <- aov(waist_hip_ratio ~ attend_cat, data = filter(baseline_df, exclude == "No"))
