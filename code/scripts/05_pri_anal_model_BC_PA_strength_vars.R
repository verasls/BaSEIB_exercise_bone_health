...
acc_peaks <- read_csv(
  ...
  select(
    subj, time, group, above_thrsh, above_thrsh_adjust , BMI_adjust, 
...
contrasts(acc_peaks$group) <- matrix(rev(contr.sum(2)), ncol = 1)
# Set contrasts of variable time to polynomial
contrasts(df$time) <- contr.poly(4)
contrasts(acc_peaks$time) <- contr.poly(4)
...
peaks_LMM <- lmer(formula = build_formula("above_thrsh"), data = acc_peaks)
