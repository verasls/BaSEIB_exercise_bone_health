...
- Dependent variable (e.g., `delta_FN_MBD`);
- `BMI_adjust`: with baseline BMI values for adjustment (significant differences between groups at baseline were found).
...
After that, linear models were fitted using the `lm()` function (`stats` package). These models had the percent change from the baseline of the BMD variables as dependent variable, attend_cat as factor and baseline BMI values as covariate. Finally, the treatment effect size was estimated using the `omega_sq()` function of the `sjstats` [package](https://cran.r-project.org/web/packages/sjstats/sjstats.pdf).
